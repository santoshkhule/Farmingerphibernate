...
public class LoginUserService {
    ...
    /**
     * Insert Operation: Fetching Object from loginUserController.java, inserting
    ...
    /**
     * Update Operation: Fetching Object from loginUserController.java, updating
     * values in login User table via session.update (requires managed entity).
     *
     * @param loginUser
     * @return boolean
     */
    ...
    /**
     * Merge Operation: Re-attaches a detached LoginUser entity (including its
     * detached ManyToMany userTypes collection) and persists changes.
     * Use this when updating a detached entity that has a ManyToMany collection.
     *
     * @param loginUser detached entity to merge
     * @return boolean
     */
    public boolean mergeLoginUser(LoginUser loginUser) {
        logger.debug("Merging LoginUser entity (id={})", loginUser.getLoginUserId());
        boolean flag = false;
        Session session = HibernateUtil.opensession();
        Transaction transaction = session.beginTransaction();
        try {
            ...
            transaction.commit();
            flag = true;
            logger.info("LoginUser merged successfully (id={})", loginUser.getLoginUserId());
        } catch (HibernateException exception) {
            if (transaction != null) {
                transaction.rollback();
            }
            logger.error("Error merging LoginUser (id={})", loginUser.getLoginUserId(), exception);
        } finally {
            session.clear();
            session.close();
        }
    ...
    /**
     * Delete Operation: Fetching Object from loginUserController.java, deleting
    ...
    /**
     * Fetch Operation: Fetching all LoginUser records from DB.
     *
     * @return list
     */
    ...
    /**
     * Fetch Operation: Fetching LoginUser by loginUserId from DB.
     *
     * @return LoginUser
     */
