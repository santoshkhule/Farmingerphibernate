...
<title><%= msg.getString("config.category.fieldset_title") %></title>
<style>
	#formPanel {
	...
	#categoryName { width:180px; padding:3px; }
...
<body>
<fieldset><legend><%= msg.getString("config.category.fieldset_title") %></legend>

	<form method="post" id="frmCategory" action="../../CategoryController">
		<input type="hidden" name="categoryId" id="categoryId">
		<div id="formPanel">
			<div id="editBanner"></div>
			<label for="categoryName"><%= msg.getString("config.category.label_category_name") %>:</label>
			<input type="text" name="categoryName" id="categoryName" required placeholder="Enter category name">
			&nbsp;
	...
	<form method="post" id="frmBulkDelete" action="../../CategoryController"></form>
	...
	<table border="1" width="100%" class="tbl-data" cellspacing="0">
		<thead>
			<tr>
				...
				<th width="8%">Id</th>
				<th><%= msg.getString("config.category.label_category_name") %></th>
		...
		<tbody>
			<%
				...
				for (CategoryEntity category : categoryList) {
					String eName = category.getCategoryName() != null ? category.getCategoryName().replace("'", "\\'") : "";
			%>
			<tr id="row-<%=category.getCategoryId()%>">
				<td style="text-align:center;"><input type="checkbox" class="rowChk" value="<%=category.getCategoryId()%>" onchange="updateBulkBar()"></td>
				<td><%=category.getCategoryId()%></td>
				<td><%=category.getCategoryName()%></td>
				<td style="text-align:center;">
					<button type="button" class="btn-row-edit"
						onclick="editRow(<%=category.getCategoryId()%>,'<%=eName%>')"><%= msg.getString("btn.edit") %></button>
				</td>
			</tr>
			<%} %>
		</tbody>
